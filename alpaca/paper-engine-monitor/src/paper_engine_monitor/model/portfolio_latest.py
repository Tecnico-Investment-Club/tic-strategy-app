...
class PortfolioLatest(State):
    """Portfolio Latest state."""

    portfolio_id: int  # entity key
    portfolio_ts: Optional[datetime]
    ...
    long_cum_rtn: Decimal
    short_rtn: Optional[Decimal]
    short_cum_rtn: Decimal
    ...
    @classmethod
    def from_source(cls, record: Record) -> "PortfolioLatest":
        """Creates object from source record."""
        res = cls()
        res.event_id = None
        res.delivery_id = None
        res.portfolio_id = record[0]  # entity key
        res.portfolio_ts = record[1] if record[1] else None
    ...
    @classmethod
    def from_target(cls, record: Tuple) -> "PortfolioLatest":
