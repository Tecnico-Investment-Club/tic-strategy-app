...
def calculate_uniform_weights(assets_to_buy, assets_to_sell, shorting_value):
    ...
    buy_array = (
        [(1 + shorting_value) / no_assets_to_buy] * no_assets_to_buy
        if no_assets_to_buy
        else []
    )
    sell_array = (
        [-shorting_value / no_assets_to_sell] * no_assets_to_sell
        if no_assets_to_sell
        else []
    )
...
def calculate_new_contributions(
    weights, evolution_df, date, previous_date, target=False
):
    """
    ...
    if target:
        ...
        new_contributions = weights.loc[previous_date] * (
            1 + evolution_df.loc[previous_date]
        )
