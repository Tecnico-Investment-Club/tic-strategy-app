...
class DistanceMethod(str, enum.Enum):
    ...
    def calculate_distance(
        method: "DistanceMethod", w_target: np.array, w_unbalanced: np.array
    ) -> float:
        """
        ...
        if method == DistanceMethod.PEARSON:
            ...
            return 1 - np.dot(
                w_target - np.mean(w_target), w_unbalanced - np.mean(w_unbalanced)
            ) / (
                np.linalg.norm(w_target - np.mean(w_target))
                * np.linalg.norm(w_unbalanced - np.mean(w_unbalanced))
            )
        elif method == DistanceMethod.CANBERRA:
            return np.sum(
                np.abs(w_target - w_unbalanced)
                / (np.abs(w_target) + np.abs(w_unbalanced))
            )
        elif method == DistanceMethod.ONE_MINUS_COSINE:
            return 1 - np.dot(w_target, w_unbalanced) / (
                np.linalg.norm(w_target) * np.linalg.norm(w_unbalanced)
            )
        elif method == DistanceMethod.BRAY_CURTIS:
            return np.sum(np.abs(w_target - w_unbalanced)) / (
                np.sum(np.abs(w_target)) + np.sum(np.abs(w_unbalanced))
            )
        else:
...
def optimize_alpha(
    ...
    w_unbalanced,
    trendy_assets_df,
    mean_reverting_assets_df,
    previous_dataframe_assets,
    previous_date,
    date,
    gamma=3,
    alpha_range=(0, 1),
    ...
    future_assets_returns = get_future_assets_returns(
        trendy_assets_df,
        mean_reverting_assets_df,
        previous_dataframe_assets,
        previous_date,
    ...
    for alpha in np.arange(alpha_range[0], alpha_range[1], step):
        ...
        filtered_weights = np.array(
            [weights_dict[asset] for asset in available_assets if asset in weights_dict]
        )

        # Select only the returns for the available assets
        filtered_returns = future_assets_returns.iloc[
            0, :
        ].values  # Only the first row of returns
        ...
        portfolio_returns = portfolio_returns_before_costs - transaction_cost * np.sum(
            np.abs(w_rebalanced - w_unbalanced)
        )
        ...
        if verbose:
            print(
                f"Alpha: {alpha}, Portfolio Returns: {portfolio_returns}, CRRA: {crra}"
            )

        if (
            best_alpha is None
            or crra > best_crra
            or (crra == best_crra and alpha > best_alpha)
        ):
            best_crra = crra
            best_alpha = alpha
            if verbose:
...
def optimize_delta(
    ...
    for delta in np.arange(delta_range[0], delta_range[1], step):
        ...
        if (
            best_delta is None
            or crra > best_crra
            or (crra == best_crra and delta > best_delta)
        ):
...
def optimize_delta_refined(
    ...
    if w_target is None or w_unbalanced is None or previous_dataframe_assets is None:
        raise ValueError(
            "Invalid inputs: w_target, w_unbalanced, and returns must be provided."
        )

    distance = DistanceMethod.calculate_distance(
        distance_method, w_target, w_unbalanced
    )
    ...
    while step >= initial_step / 1000:
        ...
        if verbose:
            print(
                f"Best delta in this iteration: {current_best_delta:.4f}, CRRA: {current_best_crra:.6f}"
            )

        if (
            np.isclose(current_best_crra, best_crra)
            and current_best_delta == previous_best_delta
        ):
            if verbose:
...
def get_future_assets_returns(
    ...
    """
    trendy_assets_prev = trendy_assets_df.loc[
        trendy_assets_df["date"] == previous_date, "asset"
    ].tolist()
    trendy_assets_curr = trendy_assets_df.loc[
        trendy_assets_df["date"] == date, "asset"
    ].tolist()
    mean_reverting_assets_prev = mean_reverting_assets_df.loc[
        mean_reverting_assets_df["date"] == previous_date, "asset"
    ].tolist()
    mean_reverting_assets_curr = mean_reverting_assets_df.loc[
        mean_reverting_assets_df["date"] == date, "asset"
    ].tolist()

    trendy_assets = list(set(trendy_assets_prev + trendy_assets_curr))
    mean_reverting_assets = list(
        set(mean_reverting_assets_prev + mean_reverting_assets_curr)
    )

    filtered_assets = trendy_assets + mean_reverting_assets
    future_assets_returns = pd.DataFrame(
        index=[previous_date], columns=filtered_assets, dtype=float
    ).fillna(0.0)
