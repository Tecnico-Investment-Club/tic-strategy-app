...
def get_sigmoid_momentum_signals(
    ...
    prices_average = indicator.prices_moving_average(
        close_prices_df.iloc[-prices_window:],
        prices_window,
        ma_type="exponential",
        exp_decay=decay,
    )
    ...
    for i, data in data_filtered.items():
        ...
        if momentum_type == dm.Momentum_Type.Cumulative_Returns:
            ...
            for asset, momentum in trendy_assets:
                if (
                    momentum > functional_constraints.get_momentum_threshold()
                ):  # buy nao significa aumentar o peso da moeda, mas que queremos ter long position, o peso vem depois
            ...
            for asset, macd_values in trendy_assets:
                ...
                if (
                    0 < macd_values[1]
                    and macd_values[3] > macd_values[5]
                    and macd_values[6] > 0
                ):  # and 0 < macd_values[4]:
                    print(
                        "close_price:",
                        close_price,
                        "macd_values[3]==histogram:",
                        macd_values[3],
                        "macd_values[5]==threshold:",
                        macd_values[5],
                    )
                    buy_and_sell_signals.setdefault(i, {}).setdefault("buy", []).append(
                        asset
...
def RSI_buy_and_sell_signalling(data_filtered):
    ...
    for i, data in data_filtered.items():
        ...
        for asset, rsi in mean_reverting_assets:
            if rsi > 70:
                buy_and_sell_signals.setdefault(i, {}).setdefault("sell", []).append(
                    asset
                )
...
def bollinger_buy_and_sell_signalling(data_filtered):
    ...
    for i, data in data_filtered.items():
        ...
        for asset, bollinger_bands in mean_reverting_assets:
            close_price = next(
                (tuple[1] for tuple in data["close_price"] if tuple[0] == asset), None
            )
            upper_band, _, lower_band = bollinger_bands
            if close_price > upper_band:
                buy_and_sell_signals.setdefault(i, {}).setdefault("sell", []).append(
...
def cumrets_mom_buy_and_sell_signalling(
    data_filtered, momentum_threshold=0.01
):  # mudar para 0.1 e colocar como parametro do example e live
    """
    Generates buy and sell signals based on momentum and Bollinger Bands.
