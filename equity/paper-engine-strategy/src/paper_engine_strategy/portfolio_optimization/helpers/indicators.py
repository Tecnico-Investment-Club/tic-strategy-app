...
def get_ATR(
    ...
    tr_data = dc.BnBClient(
        asset_ids, start_date, end_date, data_frequency
    ).get_daily_tr()
...
def calculate_bollinger_bands(df, num_std=2):
    ...
    bollinger_bands = pd.DataFrame(
        {"upper": bollinger_upper, "middle": bollinger_middle, "lower": bollinger_lower}
    )
...
def momentum_n_days(close_df, momentum_n_days=30):
    ...
    cumulative_returns = [
        (
            close_df[col].iloc[-momentum_n_days:].pct_change().add(1).cumprod().iloc[-1]
            - 1
        )
